...
At 1280px above, the content frame is capped at 1600px and centred, with a 72px icon-only left rail outside that frame, a fluid center column, a 280px right rail, and 40px gaps/padding. 768–1279px stage uses the same 72px icon-only left rail and least 16px center gutters. Small layouts use 16px gutters, 56px top brand bar, 64px bottom navigation plus safe-area inset.

*Amended 2026-08-11 (issue #59).* This section originally specified a 216px left rail, a center constrained to 640–1040px, and 24px gaps. The shipped stage uses one 72px icon rail at both desktop widths and 40px gaps, and returns the surplus to the center measure. The narrower rail was kept because the icon vocabulary then never changes between the medium and wide stages, and because identity — the connected-people count, statistics, and the Create Room panel — reads better grouped in the 280px right rail than split across both edges. The 1600px cap now applies to the content frame; the rail sits outside it, so the whole stage measures about 1685px on a 1700px monitor without horizontal overflow.

The wide rail uses icons labels cobalt inset active marker. Both desktop icon rails provide accessible names and hover/focus tooltips; an anonymous dedicated Discord affordance may use the Discord mark alone only with accessible name and tooltip `Continue with Discord`. Small bottom navigation always shows an icon and label for Home, Create, and Profile when signed in or `Discord` when anonymous. The visible anonymous label is `Discord`; its accessible name is `Continue with Discord`. Create remains the primary cobalt action, while active destination state remains distinct from destructive/live semantics. Neither responsive stage introduces a login page.
