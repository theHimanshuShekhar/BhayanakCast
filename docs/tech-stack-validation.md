...
| Full-stack framework | TanStack Start 1.168.27, React Router 1.170.17, Rsbuild 2.1.5, and Rsbuild React plugin 2.1.0 | Viable with maturity risk. Start is RC. The Router version is the direct dependency selected for the pinned Start release. Rsbuild is the documented custom-server path and supports a fetch-style Start server entry that a small Node host can forward to. |
