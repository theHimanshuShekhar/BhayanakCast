...
**Account**
: A Discord-authenticated identity used for room participation, profiles, moderation, reports, and platform enforcement. The stable Discord user ID is its sole external identity key: matching or changed provider emails never merge Accounts or transfer history. Its Discord-mirrored display name and avatar refresh on every sign-in; V1 has no local profile edit.

**Account session**
: Better Auth owns a seven-day rolling PostgreSQL-backed session whose expiry refreshes at most daily while active. Sign-out, approved account deletion, all-access restriction, or credential invalidation revokes access immediately. Intentional sign-out returns to anonymous Home; involuntary session loss follows the contextual access-gate behavior defined by Discord sign-in. A Discord Account without provider email uses a non-routable internal placeholder derived from its stable Discord ID; it is never a public/profile/search field, contact address, or verified email.

**Discord sign-in**
: The sole Account entry flow. Dedicated authentication affordances—including the Anonymous visitor's Home navigation slot where signed-in Accounts see `Profile`—use the canonical label `Continue with Discord` and start OAuth directly. An Anonymous visitor's protected action, such as `Create Room` or `Join`, keeps its action-specific label and also starts Discord OAuth directly from that click, with nearby or accessible copy disclosing Discord; it never adds an intermediate authentication step. The first successful OAuth return creates the Account and Account session immediately, with no additional onboarding or confirmation step. OAuth cancellation or failure returns to the initiating surface, preserves only its allowed non-secret intent, and shows an inline retryable error beside the initiating action. When an Account session expires or is revoked, the client immediately clears private/live state and replaces the current protected content with a contextual `Continue with Discord` access gate; it does not navigate away or restart OAuth automatically. V1 has no standalone sign-up, log-in, or authentication-error page and never sends an Anonymous visitor to Discord merely because they opened a protected destination.

**Dedicated Discord affordance**
: A provider-specific `Continue with Discord` control with the approved Discord mark beside the full text label. It follows BhayanakCast focus, contrast, sizing, theme, and disabled-state tokens rather than becoming a Discord-colored branded surface. On small Home, the Anonymous visitor sees one auth entry in the bottom-navigation slot—the mark plus `Discord`—while the top account slot remains empty until an Account is signed in. The medium icon rail may show the mark alone. Compact variants retain the accessible name and tooltip `Continue with Discord`, and every non-compact dedicated affordance shows the full canonical label. Action-specific `Create Room` and `Join` controls remain visually ordinary and identify Discord through nearby or accessible copy instead of the mark.

**Discord identity grant**
: V1 requests Discord's `identify` and `email` OAuth scopes. The stable Discord identity, display name, and avatar establish the mirrored Account identity. Provider email is optional authentication data only; it is never public, searchable, displayed, contacted, or treated as verified by BhayanakCast.

**Discord handoff state**
: The brief pending state between an OAuth-bound action and full-page navigation. Only the activated control disables, retains its dimensions, changes its visible label to `Opening Discord…`, and exposes an accessible busy state. If navigation cannot begin, the original label and enabled state return before the inline retryable error appears.

**Discord return feedback**
: A cancellation is announced once beside the restored initiating action as `Discord sign-in was cancelled`. A technical failure is announced there as `Couldn’t continue with Discord. Try again.` Neither message exposes provider codes, callback URLs, or server details. Callback error data is removed with replace-history as it is consumed, and the message clears on retry or successful navigation; reload and browser history must not resurrect it. Successful sign-in adds no confirmation toast or banner—the restored destination and signed-in Account UI are the confirmation.

**Discord return intent**
: A bounded, non-secret marker that restores the initiating surface after Discord sign-in. OAuth uses a normal full-page redirect, never a popup. Home's dedicated Discord navigation returns to Home; the Profile access gate returns to the authenticated Profile; `Join` returns one opaque Room identifier to pre-admission without auto-joining; `Create Room` returns only an opaque request to reopen a blank Create Room dialog. Anonymous `/admin` requests use the route-not-found boundary and cannot create a return intent; a Platform Admin signs in from Home before Admin navigation is exposed. Arbitrary paths, external URLs, query strings, form drafts, and private-room passwords never cross OAuth. A missing, expired, tampered, or no-longer-valid return intent fails closed to signed-in Home with one non-sensitive, one-time notice that the previous destination could not be restored; it does not discard the valid Account session.

**Profile access gate**
: The Anonymous visitor state of the Profile destination. It retains the normal lightweight Profile frame, Back/Home navigation, and `Profile` heading, then explains that Discord is required and offers `Continue with Discord`. It is not a sign-up or log-in page and contains no form, separate auth branding, or Account data.
...
**Home composition**
: Home uses three responsive stages while preserving search/filters, Live Rooms, then Past Streams. Wide (≥1280px) has persistent left identity/action and right statistics/action rails. Medium (768–1279px) keeps a compact left rail, removes the right rail, and collapses statistics with search utilities. Small (<768px) replaces both rails with a top brand bar containing `B`, connected-people count, and a signed-in avatar only when present; bottom navigation is Home/Create/Profile for an Account and Home/Create/Discord for an Anonymous visitor. Fixed small-stage bars honor safe areas, focus, and touch targets.
...
**Stream Preview**
: A non-subscribed indication that a Stream is live. Public-room previews use an unblurred thumbnail; private-room previews use a blurred thumbnail, and that blur is a property of the stored image rather than of its styling—a private preview is stored at most 64px wide against 640px for a public one, with the server reading the WebP header to enforce the cap. A room turning private retires the previews its live Streams stored under the public rule. TanStack Pacer `useAsyncThrottler` allows an immediate first upload then latest-value trailing uploads at most every two minutes; stop/leave/unmount cancels pending work and aborts in-flight upload. Preview state is live-only and may be frozen as report evidence.
