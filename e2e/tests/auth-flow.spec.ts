...
test.describe("Example: Streamer and Viewer Test Flow", () => {
  test("streamer can create room and viewer can join", async ({
    ...
    try {
      ...
      // Use fixed room name and fill form
      const roomName = "Stream Room Test";
      await streamerPage.getByPlaceholder("Enter room name...").fill(roomName);
      await streamerPage.getByRole("textbox", { name: "Description (Optional)" }).fill("A test room for E2E testing");
      ...
      // Verify streamer is in the room (check for partial match since name might be censored)
      await expect(streamerPage.locator("h1")).toContainText("Room");
      ...
      await expect(viewerPage.locator("h1")).toContainText("Room");
  ...
  test("multiple viewers can join a stream", async ({ browser, signupTestUser }) => {
    ...
    try {
      ...
      await streamerPage.getByPlaceholder("Enter room name...").fill("Multi Viewer Test");
      await streamerPage.click('button[type="submit"]:has-text("Create Room")');
      await streamerPage.waitForURL(/\/room\/.+/, { timeout: 10000 });
